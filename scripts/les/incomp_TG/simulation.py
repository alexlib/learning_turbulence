...
# Closure
closure_cls = getattr(dedaLES.closures, param.closure_cls)
closure = closure_cls(*param.closure_args)
...
for i in ['x', 'y', 'z']:
    for j in ['x', 'y', 'z']:
        problem.substitutions[f'u{i}{j}'] = f"d{j}(u{i})"
dedaLES.closures.add_closure_substitutions(problem, closure, u='ux', v='uy', w='uz')
print(problem.substitutions)
dedaLES.closures.add_closure_equations(problem, closure, u='ux', v='uy', w='uz')
